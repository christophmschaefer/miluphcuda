...
# if DISPH
        ...
        if (q_id < 0) {
            fprintf(stderr, "Could not find q information in hdf5 file.  Exiting\n");
            exit(1);
        }
        x = (double * ) malloc(sizeof(double) * my_anop);
        status = H5Dread(q_id, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT, x);
        status = H5Dclose(q_id);
...
# if DISPH
        /* read internal energies */
        U_id = H5Dopen(file_id, "/U", H5P_DEFAULT);
        if (U_id < 0) {
            fprintf(stderr, "Could not find U (DISPH) information in hdf5 file.  Exiting\n");
            exit(1);
        }
        x = (double * ) malloc(sizeof(double) * my_anop);
        status = H5Dread(U_id, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT, x);
...
<<<<<<< HEAD
            ...
            // read in q
            if (!fscanf(inputFile.data, "%s", &iotmp))
                fprintf(stderr, "could not read q from input file\n");
            p_host.q[i] = atof(iotmp);
            columns++;
#endif
...
<<<<<<< HEAD
            ...
            // read in U
            if (!fscanf(inputFile.data, "%s", &iotmp))
                fprintf(stderr, "could not read U from input file\n");
            p_host.U[i] = atof(iotmp);
            columns++;
#endif
...
<<<<<<< HEAD
	        ...
	        status = H5Dwrite(q_id, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT, x);
	        status = H5Dclose(q_id);

	        /* U */
	        U_id = H5Dcreate2(file_id, "/U", H5T_NATIVE_DOUBLE, dataspace_id,
	                H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
	        for (i = 0; i < numberOfParticles; i++)
	            x[i] = p_host.U[i];

	        status = H5Dwrite(U_id, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT, x);
	        status = H5Dclose(U_id);
        #endif
...
<<<<<<< HEAD
        ...
        /* change of U */
        x = (double *) malloc(sizeof(double) * numberOfParticles);
        dUdt_id = H5Dcreate2(file_id, "/dUdt", H5T_NATIVE_DOUBLE, dataspace_id,
                H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
        for (i = 0; i < numberOfParticles; i++)
            x[i] = p_host.dUdt[i];

        status = H5Dwrite(dUdt_id, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT, x);
        status = H5Dclose(dUdt_id);
        free(x);
#endif
