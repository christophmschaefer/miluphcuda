...
int main(int argc, char *argv[]) {
    ...
    strcpy(param.accretionfilename, "accretion_parameters.log");
    ...
    int i, c;
    while ((c = getopt_long(argc, argv, "Q:d:M:b:m:L:k:T:DI:t:a:n:f:F:c:e:b:o:rXYvhHshVgGA", opts, &i)) != -1) {
        switch (c) {
            case 'F':
            ...
            case 'e':
                ...
                break;
            case 'o':
                if( !strcpy(param.accretionfilename, optarg) ) {
                    fprintf(stderr, "Something's wrong with the name of the logfile for accretion parameters.\n");
                    exit(1);
                }
                break;
            case 'n':
...
#if FIXED_BINARY
    /* Create accretion parameters file and write header */
    if(param.hdf5input){
        if( (param.accretionfile = fopen(param.accretionfilename, "a")) == NULL) {
            fprintf(stderr, "Cannot open %s for appending. Aborting ...\n", param.accretionfilename);
            exit(1);
        }
    }
    else {
        if( (param.accretionfile = fopen(param.accretionfilename, "w")) == NULL ) {
            fprintf(stderr, "Cannot open %s for writing. Aborting ...\n", param.accretionfilename);
            exit(1);
        }
        fprintf(param.accretionfile, "# Format for accretion log file: <column>:<datavalue>\n"); 
        fprintf(param.accretionfile, "# M_xxx accreted mass by star1, star2, inner boundary, outer boundary, same naming convection for the other values.\n"); 
        fprintf(param.accretionfile, "# these values are cumulated between the different time outputs.\n");
        fprintf(param.accretionfile, "# J_grav1 and J_grav2 is the z-torque exerted on star1,2 by the particles.\n");
        fprintf(param.accretionfile, "# these values are the current values from the last rhs call.\n");
        fprintf(param.accretionfile, "# 1:time 2:M_acc1 3:M_acc2 4:M_in 5:M_out 6:J_acc1 7:J_acc2 8:J_in 9:J_out 10:N_acc1 11:N_acc2 12:N_in 13:N_out 14:J_grav1 15:J_grav2\n");
    }
#endif
