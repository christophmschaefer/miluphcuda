...
for i in range(int(np.floor(np.sqrt(N1)))):
    for j in range(int(np.floor(np.sqrt(N1)))):
        ...
        if x < 0.5 and y > 0.9:
            print("%e %e %e %e %e 0 u0" % (x, y, v1, v_trigger,  m1), file=outputf)
        else:
            print("%e %e %e %e %e 0 u0" % (x, y, v1, v1, m1), file=outputf)
...
for i in range(int(np.floor(np.sqrt(N2)))):
    for j in range(int(np.floor(np.sqrt(N2)))):
        ...
        if x > 0.5 and y < 1.1:
            print("%e %e %e %e %e 1 u0" % (x, y, v2, -v_trigger, m2), file=outputf)
        else:
            print("%e %e %e %e %e 1 u0" % (x,y, v2, v2, m2), file=outputf)
...
for i in range(len(x_bp)):
    print("%e %e 0.0 0.0 %e 2 0.0" % (x_bp[i], y_bp[i], m_bp), file=outputf)
