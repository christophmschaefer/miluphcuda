    ...
    for (i = threadIdx.x + blockIdx.x * blockDim.x; i < numParticles; i += inc) {
        ...
        if (EOS_TYPE_POLYTROPIC_GAS == matEOS[matId]) {
            ...
            if (i_e < 0 && p.e[i] >= aneos_e_c[aneos_e_id_c[matId] + aneos_n_e_c[matId] - 1]) {
                ...
                p.cs[i] = sqrt(aneos_gamma_c[matId] * (aneos_gamma_c[matId] - 1.0) * p.e[i]);
            } else if (i_e < 0) {
                // e below table minimum: clamp to cold curve
                i_e = 0;
                p.cs[i] = bilinear_interpolation_from_linearized(p.rho[i], aneos_e_c[aneos_e_id_c[matId]], aneos_cs_c+aneos_matrix_id_c[matId], aneos_rho_c+aneos_rho_id_c[matId], aneos_e_c+aneos_e_id_c[matId], i_rho, i_e, aneos_n_rho_c[matId], aneos_n_e_c[matId], i);
            } else {
...
//            }
            ...
            if (i_e < 0 && p.e[i] >= aneos_e_c[aneos_e_id_c[matId] + aneos_n_e_c[matId] - 1]) {
                // e above table maximum: ideal gas sound speed
                cs = sqrt(aneos_gamma_c[matId] * (aneos_gamma_c[matId] - 1.0) * p.e[i]);
            } else if (i_e < 0) {
                // e below table minimum: clamp to cold curve
                i_e = 0;
