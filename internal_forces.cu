...
# endif // ARTIFICIAL_STRESS
            ...
            for (d = 0; d < DIM; d++) {
                accelsj[d] =  -p.U[i]*p.U[j]/p.m[i] * (p.p[i]/(p.q[i]*p.q[i]) + p.p[j]/(p.q[j]*p.q[j])) * dWdx[d];
                accels[d] += accelsj[d];
            }
#else
...
# if ARTIFICIAL_VISCOSITY
            ...
            dUdt += 0.5 * p.m[i]*p.m[j] * pij * vvnablaW;
...
// dUdt for non-solid
            ...
            dUdt += 0.5 * p.m[i] * accelsj[0] * -dvx;
#  if DIM > 1
            dUdt += 0.5 * p.m[i] * accelsj[1] * -dvy;
#  endif
#  if DIM > 2
            dUdt += 0.5 * p.m[i] * accelsj[2] * -dvz;
#  endif
